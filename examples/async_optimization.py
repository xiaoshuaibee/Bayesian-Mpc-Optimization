...
try:
    ...
    raise ImportError(
        "In order to run this example you must have the libraries:为了运行这个例子，你必须有这些库： " +
        "`tornado` and `requests` installed."
    )


def black_box_function(x, y):
    """Function with unknown internals we wish to maximize.我们希望最大化内部未知的函数

    This is just serving as an example, however, for all intents and
    purposes think of the internals of this function, i.e.: the process
    which generates its outputs values, as unknown.
    这只是作为一个例子，然而，考虑到这个函数的内部，即:产生输出值的过程，是未知的
    """
...
class BayesianOptimizationHandler(RequestHandler):
    """Basic functionality for NLP handlers.NLP处理程序的基本功能"""
    ...
    def post(self):
        """Deal with incoming requests.处理传入的请求"""
