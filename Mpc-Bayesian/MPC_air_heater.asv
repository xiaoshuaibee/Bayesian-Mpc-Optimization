...
% A_est 是一个 2x2 的矩阵，表示估计器的状态转移矩阵。它基于系统参数和随机噪声的影响。
% C_est 是一个 1x2 的矩阵，表示观测器的输出矩阵，用于从系统状态中提取观测量。
% n_est 是状态转移矩阵的阶数，这里是 2。
% Tr_est 是观测器的所需上升时间。
% T_est 是观测器的采样时间，通过将所需上升时间分配给状态矩阵的阶数而计算得到。
% estimpoly 是一个长度为 3 的向量，其中包含了所需的观测器特征多项式的系数。
% eig_est 是根据观测器特征多项式计算得到的期望特征值。
% K_est1 使用极点放置法计算观测器增益矩阵，确保了观测器的期望响应。
% 最后，K_est 是通过转置 K_est1 得到的观测器增益矩阵。
