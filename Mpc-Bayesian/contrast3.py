...
def bayesopt_objective(**kwargs):
    ...
    try:
        error, _ = eng.calculate_error2(gain, theta_const, theta_delay, control_inputs_matlab, nargout=2)
...
if __name__ == "__main__":
    ...
    theta_const = 3.0
    theta_delay = 1.0
    baseline_control_input = 110  # 基准控制电压
    _, baseline_error = eng.calculate_error2(gain, theta_const, theta_delay,
                                             matlab.double([baseline_control_input] * N_pred), nargout=2)

    # 打印对比结果
    print("\n", "Best score with Bayesian Optimization:", -best_score)
    print("Baseline control error:", baseline_error)
    if -best_score < baseline_error:
        print("Bayesian Optimization performs better.")
    else:
        print("Baseline control performs better.")
